...
const ClientConfig = ({ userDetail, menuList, handleCancel, updateClientConfig, fontFamily }) => {
    ...
    const fetchClientMenuConfig = async () => {
        try {
    ...
    const handleSubmitOrUpdate = () => {
        ...
        updateClientConfig({ menuIds: config, email: userDetail.email });
    };
    ...
    return (
        <>
            ...
            <Panel style={{
                ...
                <Heading level={3} style={{ textAlign: 'center', fontWeight: 'bold' }}>
                    Client Configuration
                </Heading>

                {Object.entries(groupedMenus).map(([group, menus]) => (
                    <Panel key={group} bordered style={{ marginTop: 20 }}>
                        <Heading
                            level={5}
                            style={{
                                textAlign: 'center',
                                ...
                                <Cell>
                                    {(rowData) => (
                                        <Checkbox
                                            ...
                                            onChange={(value, checked) => handleMenuCheckboxChange(rowData.id, checked)}
                                        />
                                    )}
                                </Cell>
                            </Column>
